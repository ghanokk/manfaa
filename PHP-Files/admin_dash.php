...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_role_user_id'])) {
    ...
    // Map role names to role_ids (1=student, 2=instructor, 3=admin)
    $roleMap = ['student' => 1, 'instructor' => 2, 'admin' => 3];
    $newRoleId = $roleMap[$newRole] ?? 1;
    
    if ($changeUserId !== $userId) { // Can't change your own role
        // Delete existing roles for this user
        $deleteSql = "DELETE FROM user_roles WHERE user_id = ?";
        $deleteStmt = $conn->prepare($deleteSql);
        $deleteStmt->bind_param('i', $changeUserId);
        $deleteStmt->execute();
        $deleteStmt->close();
        ...
        $insertStmt->bind_param('ii', $changeUserId, $newRoleId);
        $insertStmt->execute();
        $insertStmt->close();
...
<body>
    <header class="site-header">
        <nav class="navbar">
            ...
            <div class="nav-right">
                <a class="tab-btn active" class="link" href="dashboard.php">Dashboard</a>
                <a class="tab-btn active" class="link" href="logout.php">Logout</a>
            </div>
        </nav>
    </header>
